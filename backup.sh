...
echo -n "Deseja iniciar/continuar o backup? [1-sim / 2-não]:"
read repete
while [ $repete -eq 1 ]; do
      #mostra opções de backup
      ...
      if lsblk -f | grep -wq $uuid; then
        echo "HD Conectado com sucesso!"
          if [ $op -eq 1 ]; then
            ...
            #implementa comando shell com rsync para realização do backup
            #Atenção ao diretório que será copiado no backup
